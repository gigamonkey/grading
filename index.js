...
app.put('/prompt-responses/:assignmentId/grade', (req, _res) => {
  console.log(req.body);
  db.postPromptResponseGrade(req.body);
});

app.get('/coding', async (_req, res) => {
  const a = loadJSON(path.join(dir, 'assignment.json'));
  const students = Object.fromEntries(
    db.studentsForPeriod({ period: 5 }).map((s) => {
      return [s.github, s.user_id];
    }),
  );
  const files = Object.fromEntries(
    glob.sync(path.join(dir, `*/${a.file}`)).map((p) => {
