# TODO

## Quick

Items here can be done directly in the current branch. The user will tell you
when to commit.

## Small

Items here should be done in a branch which you should create. You may commit to
the branch whenever you wish but the user will tell you when to merge the branch
back into `main`.

- [ ] Update `schema-cleanup.md` based on the current schema and code.

## Medium

Items here should be done in a worktree which you should create. You may commit
to the branch in the worktree whenever you wish but the user will tell you when
to merge the worktree branch back into `main` and clean up the worktree.

- [ ] Add a link on each student from the `/checklists/:assignment_id` page to a
      page that shows the students name, points, and score at the top of the
      page and then a table of the criteria with checkmarks, xs, and blanks the
      same as that student's row in the checklist assignment page.

- [ ] Examine whether `standard` should be part of the `assignment_point_values`
      PK. Currently `(assignment_id, standard)` is the PK, which was intended to
      allow an assignment to have different point values for different standards,
      but that's never been used in practice. Having standard in the PK makes
      per-field inline editing awkward since you can't save points or IC name
      until a standard is set. Consider removing standard from the PK and
      separating concerns — e.g. a simpler table for assignment config (points,
      ic_name) with standard as a regular column. The big question: should a
      single assignment ever be associated with more than one standard?

- [ ] Change schema of `checklist_marks` to have a foreign key with delete on
      cascade referencing `checklist_criteria`.

- [ ] Change the schema of `checklist_criteria` and `checklist_marks` so the key
      of `checklist_criteria` is an autogenerated `INTEGER`. Keep the `seq`
      column but it will not be part of the key so we can use it to order the
      criteria on the checklist page and potentially can reorder them by
      changing their `seq` values.

- [ ] Implement auto scoring of expressions assignments.

## Needs plan

Items in here should result in a new plan file in `plans/`. Feel free to ask
questions about requirements if the item is not clear enough about what is
wanted.

- [ ] Write a plan for making `standards` a first class thing in the database
      with a UI for adding them.

- [ ] Make a plan for cleaning up scripts whose functionality exists in the
      `app.js` grading server or should. Probably start by updating
      `reports/script-cleanup.md` focusing on scripts that are no longer needed
      giving the grading server and ones whose functionality should be moved
      into the grading server.

## Large

- [ ] Unify Java and Javascript speedrun and history testing scripts. Make
      Speedrun.java and modules/speedrun.js both spit out JSON data in the same
      format containing the history of the speedrun and then write a single
      Javascript script that can invoke either of them as appropriate and then
      is responsible for rendering the history in a standard way (e.g. showing
      the elapsed time, number of tests passed at each change, etc.)

## Plans

This is where you put new items to actually implement a plan, after you haved
written the plan.

## Done

Move newly done items to the **bottom** of this list so is is kept in
chronological order from oldest to newest.


- [x] On the `/checklists` page, deleting a criterion column should update the
      points and scores for the whole table.

- [x] Add filters on all columns with low-cardinality. For instance on the `IC
      name` column in the table on the `/to-update` page, clicking on the column
      header should pop up a menu where the user can select one or more specific
      values to filter the table down to. Should be links or buttons for "Clear
      all" and "All" which clear all selections and select them all
      respectively. Then clicking on individual names selects or deselects them.

- [x] Please make the boxes on the home page links to the corresponding section
      of the site.

- [x] On the `/checklist` page in `app.js` show all assignments in a table And
      and use the search box to narrow the view just like on the assignments
      page. However only show assignments that haven't already been graded in
      some way.

- [x] Make it so any any click-to-edit elements such as on the checklist grading
      page, hitting Esc while the element is edit mode exits edit mode, i.e.
      turns an input back into plain text.

- [x] Write a plan for adding a feature to the website that combines the
      functionality of `get-file.js`, `load-answers.js`, and `score-answers.py`
      for scoring answers to a quiz. It does not need to write anything to files
      like `get-file.js` does but it should fetch the `answers.json` files from
      repos under `$BHS_CS_REPOS` the same way `get-file.js` does. Then it
      should load the answers into the db the way `load-answers.js` does.
      Finally, there should be a page on the website for scoring all the answers
      submitted per question similar to the way `score-answers.py` does but
      rather than asking one at a time, it should present each question and all
      the unique answers to that question (with a number showing how many
      students gave that answer) and a place to click to mark the answer as
      correct or incorrect for choices, ochoices, mchoices, and omchoices and a
      place to enter the score for freeanswers. When all answers are scored it
      should advance to the next question but there should be nav elements for
      moving through the set of questions. (plan: [quiz-scoring.md](plans/quiz-scoring.md))

- [x] Implement the plan in [plans/quiz-scoring.md](plans/quiz-scoring.md)

- [x] Make it so when adding an assignment the drop down to pick the standard
      also allows the user to enter a new standard name.

- [x] There seems to be a bug in the `/checklists` page where if there isn't a
      criterion named `"Turned in"` one gets added when we sort the table or
      reload. While the first time we load an assignment it should create a
      "Turned in" criterion, if it gets renamed there's no need to create a new
      one.
