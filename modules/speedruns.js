...
const showCommit = (sha, timestamp, totalElapsed, elapsed, passed, attempted) => {
  ...
  try {
    ...
    console.log(
      `${shortSha}: ${date} (${totalTime} +${commitTime}) - Passed: ${passed} of ${attempted}`,
    );
  } catch (e) {
...
const summarizeBranch = (repoDir, path, file, testcases, branch, questions) => {
  ...
  console.log(
    `Total time: ${totalTime}; max passed: ${maxPassed} of ${questions}. ${commits.length} commits. ${errors} errors`,
  );
...
/**
 * Return structured commit-by-commit data for a JavaScript speedrun.
 * Same analysis as showCommits but returns data instead of logging.
 */
const getCommitData = (repoDir, path, file, testcases, start, end, branch, questions) => {
  const repo = new Repo(repoDir);
  let maxPassed = 0;
  const rawCommits = repo.changes(start, end, branch);

  const commits = rawCommits.map((c, i, arr) => {
    const { sha, timestamp } = c;
    const totalElapsed = timestamp - arr[arr.length - 1]?.timestamp ?? 0;
    const elapsed = i < arr.length - 1 ? timestamp - arr[i + 1]?.timestamp : 0;
    const code = repo.contents(sha, `${path}/${file}`);
    ...
    if (error) {
      return {
        ...
        timestamp,
        date: dateFormat(Instant.fromEpochMilliseconds(timestamp * 1e3)),
        totalElapsed: durationString(Duration.from({ seconds: totalElapsed })),
        elapsed: durationString(Duration.from({ seconds: elapsed })),
  ...
  const totalSeconds =
    rawCommits.length > 0
      ? rawCommits[0].timestamp - rawCommits[rawCommits.length - 1].timestamp
      : 0;

  return {
    commits,
    totalTime: durationString(Duration.from({ seconds: totalSeconds })),
    totalSeconds,
    maxPassed,
    questions,
  };
...
export { showCommits, showBranch, summarizeBranch, analyzeSpeedrun, getCommitData, durationString };
